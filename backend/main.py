...
def get_words_for_review(language: str, limit: int = 10):
...
async def create_speaking_activity(language: str, request: Request):
    ...
    try:
        ...
        print(f"✓ Activity saved immediately after generation for {language}")
...
async def create_conversation_activity(language: str, request: Request):
    ...
    try:
        ...
        # Update activity_data with conversation_id and save again
        if conversation_id:
...
def update_flashcard_word(request: FlashcardUpdateRequest):
    ...
    try:
        ...
        return {"success": True, "message": "Word state updated"}
    except Exception as e:
...
def get_user_languages():
    ...
    # Check if user has language preferences saved
    cursor.execute('''
        SELECT value FROM user_preferences
...
async def save_user_languages(request: Request):
    ...
    # Save or update language preferences
    cursor.execute('''
...
async def save_user_interests(request: Request):
    ...
    # Save or update interests preferences
    cursor.execute('''
        INSERT OR REPLACE INTO user_preferences (user_id, key, value, updated_at)
