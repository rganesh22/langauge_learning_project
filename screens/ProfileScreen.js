...
export default function ProfileScreen() {
  ...
  const [profileLanguage, setProfileLanguage] = useState(ctxLanguage || 'kannada');
  ...
  const loadSelectedLanguages = async () => {
    try {
      ...
      // Default to active languages from LANGUAGES
      const activeLanguages = LANGUAGES.filter(l => l.active).map(l => l.code);
  ...
  const toggleLanguageSelection = async (langCode) => {
    ...
    try {
      ...
      await fetch(`${API_BASE_URL}/api/user-languages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ languages: newSelection })
      });
      
      // Reload user languages in context so all screens update
