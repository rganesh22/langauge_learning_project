...
const ACTIVITY_ORDER = ['flashcards', 'transliteration', 'reading', 'listening', 'writing', 'speaking', 'translation'];
...
export default function DashboardScreen({ navigation }) {
  ...
  }, [selectedLanguage, weekOffset, availableLanguages.length]);
  ...
  const startActivity = (activityType, language = null) => {
    ...
    if (activityType === 'flashcards') {
      navigation.navigate('Flashcards', { language: targetLanguage });
    } else {
      navigation.navigate('Activity', {
  ...
  // No language selected yet — show empty state immediately
  if (!selectedLanguage || availableLanguages.length === 0) {
    return (
      <View style={styles.container}>
        ...
        <NoLanguageEmptyState message="Select a language in your Profile to see your dashboard." />
      </View>
    );
  }
